...
void generation_aleatoire(Terrain *T, int l, int h, float dObst) {
  /* À compléter */
  int x, y, random, obstacle, conteur;

  T->largeur = l;
  T->hauteur = h;

  if(T==NULL){
    printf("Terrain inexistant\n");
    exit(EXIT_FAILURE);
  } 
  ...
  obstacle = (int) l*h*dObst;
  conteur = 0;

  /* Tant que le nombre d'obstacle correspond a la demande*/
  while(conteur!=obstacle){
    ...
    if(!(T->tab[x][y]==ROCHER || T->tab[x][y]==EAU)){
      ...
      T->tab[x][y] = (random==0) ? EAU : ROCHER;
      conteur++; //seulement si on fait un changement sur la case
    } 
  } 
}
